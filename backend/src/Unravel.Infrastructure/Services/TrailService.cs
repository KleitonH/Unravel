...
public class TrailService : ITrailService
{
    ...
    {
        var total = await _db.Content.CountAsync(c => c.TrailId == trailId && c.IsActive);
        if (total == 0) return 0;
    ...
    {
        var trail = await _db.Trail.FindAsync(id);
    ...
    {
        var content = await _db.Content.FindAsync(contentId)
            ?? throw new KeyNotFoundException("Conteúdo não encontrado.");

        var uc = await _db.UserContent

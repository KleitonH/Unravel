...
function IslandWorldMap({
  ...
  // base do castelo encosta na grama (afunda um pouco); castelo fica À FRENTE.
  const groundTop = bossY + g.BOSS * 0.34
...
function BossCastle({ x, y, g, unlocked, onOpen }: { x: number; y: number; g: Geom; unlocked: boolean; onOpen: () => void }) {
  ...
  return (
    <div style={{ position: "absolute", left: `${x}%`, top: y, width: D, height: D, transform: "translate(-50%,-50%)", zIndex: 11 }}>
      {/* sombra atrás do castelo, junto à base */}
      <div style={{ position: "absolute", left: "50%", top: D * 0.88, transform: "translate(-50%,-50%)", width: D * 0.85, height: 20, background: "radial-gradient(ellipse, rgba(0,0,0,0.55) 55%, rgba(0,0,0,0) 80%)", borderRadius: "50%", pointerEvents: "none" }} />

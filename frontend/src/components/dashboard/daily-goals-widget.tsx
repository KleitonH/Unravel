...
import { questsApi, type DailyQuest } from "@/api/quests"
...
export function DailyGoalsWidget() {
  ...
  const quests       = useQuery({ queryKey: ["quests", "today"], queryFn: questsApi.today, staleTime: 60_000 })

  const novelos = (partnerships.data ?? []).filter((p) => p.yarn).slice(0, 2)
  const cx      = caixinha.data ?? null
  const todays  = quests.data ?? []

  const loading = partnerships.isLoading || caixinha.isLoading || quests.isLoading
  if (loading) return <Skeleton className="h-52 w-full" />
  if (novelos.length === 0 && !cx && todays.length === 0) return null
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
        ...
        {/* Missões do dia — completar cada uma alimenta novelo + clã. */}
        {todays.length > 0 && (
          <div className="rounded-md border border-dashed border-border p-3">
            <div className="mb-2 flex items-center justify-between">
              ...
              <span className="text-[11px] tabular-nums text-muted-foreground">
                {todays.filter((q) => q.completed).length}/{todays.length}
              </span>
            </div>
            <ul className="space-y-1.5">
              {todays.map((q) => <QuestRow key={q.key} q={q} />)}
            </ul>
            <Link
              to="/trails"
...
/** Linha de missão: ícone + título + barra de progresso (ou ✓ quando completa). */
function QuestRow({ q }: { q: DailyQuest }) {
  const pct = Math.min(100, Math.round((q.progress / Math.max(q.target, 1)) * 100))
  return (
    <li className={cn("flex items-center gap-2.5", q.completed && "opacity-70")}>
      <span
        className={cn(
          "grid h-7 w-7 shrink-0 place-items-center rounded-full text-sm",
          q.completed ? "bg-success/15" : "bg-foreground/5",
        )}
      >
        {q.completed ? <Check className="h-3.5 w-3.5 text-success" /> : q.icon}
      </span>
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between gap-2">
          <span className={cn("truncate text-xs font-medium", q.completed && "line-through")}>
            {q.title}
          </span>
        ...
        <div className="mt-1 h-1 w-full overflow-hidden rounded-full bg-border">
          <div
            className={cn("h-full rounded-full transition-all", q.completed ? "bg-success" : "bg-primary")}
            style={{ width: `${pct}%` }}
          />
        </div>
      </div>
